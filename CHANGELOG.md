...
## 2019.11.05
- `mitv` 新增小米电视 5 系列（小米电视 5 Pro 型号稍后补充）。
## 2019.11.04
...
- `honor` 新增荣耀 V20。
## 2018.12.24
- `xiaomi` 个别机型诸如红米 4 高配版（`B5W`, `markw`）、红米 Note 4（`B6L`, `nikel`）在内部代码（此处均指 MIUI 系统中的代码）中的代号为 `mark` / `nike`，后面多出来的字母出自 `B5W` / `B6L` 中的最后一个字母。部分机型如红米 1 系列、红米 2 系列、红米 Note 5A 标准版等拥有多个内部代号（如红米 2A 高配版同时拥有 `wt86047`、`HM2014816`、`thomas` 三个代号），为避免可能的混淆，此汇总的产品机型代号将以 MIUI 官网可查到的代号（即线刷包中的代号）为准。
## 2018.12.21
- `xiaomi` 新增小米 Play。
## 2018.12.20
