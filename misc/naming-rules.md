...
`四位数字` + `五位识别码` + `一位字母`
- `四位数字`: 前二位代表年份，后二位代表月份，表示该产品开售（不是发布）时间不早于该日期
- `五位识别码`: 由字母和数字构成。第二、三位代表产品系列，`PN` - Xiaomi 数字/Civi，`PX` - Xiaomi MIX，`PY` - Xiaomi Civi，`RA` - Redmi Note，`RT` - Redmi Turbo, `RN` - Redmi 数字，`RK` - Redmi K，`PC` - POCO，`RP` - Xiaomi/Redmi 平板
