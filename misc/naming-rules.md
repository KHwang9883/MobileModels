...
### 型号命名规则
`A` + `四位数字`
- `四位数字`: 无明显规律，但通常数字越大推出时间越晚（可能有反例）
...
### 传播名注意事项
华为大部分产品传播名中的品牌名称均为六个大写字母「HUAWEI」
...
- `一位字母`: 区分渠道或网络版本，`Q` - 全网通公开版；`M` 或 `Y` - 移动版；`U` - 联通版；`C` 或 `D` - 电信版；`A` - YunOS 版；`H` - 国际版；空缺代表双网通公开版
...
- `四位数字`: 区分 SKU，前二位代表年份；第三位数字越大，在系列内定位越高；第四位用于区分地区/运营商版本，`0` - 国行，`1` - 印度版，`3` - 欧版/国际版，`5` - 北美版（个别产品可能有出入）
- **示例:** `LE2120`（OnePlus 9 Pro 国行全网通版），其中 `LE` 为系列产品代号 `lemonade` 缩写；`21` 代表 2021 年，第四位 `0` 代表国行全网通版

### 传播名注意事项
产品传播名中，品牌名无论叫「一加」还是叫「OnePlus」都没有问题，但在重新并入 OPPO 之前，官方宣发中品牌名为「OnePlus」
...
### 传播名注意事项
Reno/Ace 系列，系列名与数字之间无空格

## realme（真我）
### 型号命名规则
`RMX` 或 `RMP` + `四位数字`
- `RMX` 为 realme 手机，`RMP` 为平板
- `四位数字`: 区分产品，无明显规律，但通常前二位数字越大，产品推出时间越晚
...
- `一位字母`: 区分渠道版本，`A` `E` - 全网通版，`C` - 全网通版（2020 及之后）或运营商定制版（2019 及之前），`G` `H` `Y` - 国际版，`I` `P` - 印度版，`L` - 拉丁美洲版，`R` - 日版，`T` - 运营商定制版或泰国版
- **示例:** `M2102K1AC`（小米 11 Pro 全网通版），其中 `2102` 表示该产品上市时间不早于 2021 年 2 月；`K1A` 为产品内部编号；`C` 代表全网通版

### 型号命名规则（2021-）
`四位数字` + `XXX` + `一位字母`
- `四位数字`: 前二位代表年份，后二位代表月份，表示该产品上市时间不早于该日期
- `XXX`: 通常由几位数字及后缀字母构成，为产品内部编号；前一或二位数字代表第几个英文字母；剩余部分与产品系列之间有规律关系
- `一位字母`: 区分渠道版本，`C` - 全网通版，`G` `H` `Y` - 国际版，`I` `P` - 印度版，`L` - 拉丁美洲版，`R` - 日版，`T` - 运营商定制版或泰国版
- **示例:** `21091116C`（Redmi Note 11 Pro 全网通版），其中 `2109` 表示该产品上市时间不早于 2021 年 9 月；`1116` 表示产品内部编号为 `K16`；`C` 代表全网通版

### 传播名注意事项
2021 年 Q3 后，小米品牌产品传播名中的品牌名称为六个字母「Xiaomi」（X 大写，其他字母小写），例如 `Xiaomi MIX 4`；Redmi 品牌独立后，除数字系列与部分 IoT 产品外，官方宣发通常不使用「红米」二字

## 黑鲨
### 型号命名规则
`三位字母` + 横杠 + `A0` 或 `H0`
- `三位字母`: 区分产品，为产品研发代号缩写
- `A0` 代表国行全网通版，`H0` 代表国际版
