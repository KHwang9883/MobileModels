...
- **注:** 2023 年后推出的华为「5A 速度」产品仍沿用 4G 制式的型号命名
...
### 型号命名规则（2018 年起）
`V` 或 `I` 或 `J` + `四位数字` + `一位字母`
- `V` 代表 vivo，`I` 代表 iQOO（仅海外版），`J` 代表 JOVI（部分市场）
...
- **注:** 国行版型号 `四位数字` 通常与机型代号相关；海外版型号 `四位数字` 与国行无对应关系，亦与机型代号无关
...
- **示例:** `M2102K1AC`（小米 11 Pro），其中 `2102` 表示该产品开售时间不早于 2021 年 2 月；`K1A` 为产品项目代号；`C` 代表国行全网通版
...
- **示例:** `21091116C`（Redmi Note 11 Pro），其中 `2109` 表示该产品开售时间不早于 2021 年 9 月；`1116` 表示产品项目代号为 `K16`（前二位代表第 11 个英文字母 K）；`C` 代表国行全网通版

### 型号命名规则（国行 2022-2026，国际 2022-至今）
`四位数字` + `五位识别码` + `一位字母`
- `四位数字`: 前二位代表年份，后二位代表月份，表示该产品预计上市时间不早于该日期
- `五位识别码`: 由字母和数字构成。
    - 第一、四、五位为产品项目代号/主板代号（2026 起，全大写或首字母大写）的 SHA-1 哈希值的前三位；若之后发布产品的代号哈希值与已有的重复，则取哈希值中第 2~4 位
...
- **示例:** `22127RK46C`（Redmi K60 Pro），其中 `2212` 表示该产品开售时间不早于 2022 年 12 月；`RK` 代表 Redmi K 系列，`746` 是项目代号 `M11` 的 SHA-1 哈希值的前三位；`C` 代表国行全网通版

### 型号命名规则（国行 2026 年起）
`M` + `三位数字` + `二位字母`
- `三位数字` `二位字母` 均取自产品项目代号（极个别）/主板代号（全大写或首字母大写）的 SHA-1 哈希值，其中 `三位数字` 是这串哈希值的第 1、2、3 个数字先后排列，`二位字母` 是第 1、2 个字母先后排列。
- **示例:** `M025EC`（Xiaomi 17T Pro 国行版），取该机型项目代号 `P12U` 的 SHA-1 哈希值 `ec02ad5b...` 的前三位数字和前二位字母。
